...
export function createAlteaServer({ makeClient, log = (m) => process.stderr.write(`[altea-mcp] ${m}\n`), readTimeoutMs = 60_000, actionTimeoutMs = 150_000, idleMs = 10 * 60_000 } = {}) {
  ...
  server.registerTool('altea_waitlist', {
    ...
    description: 'Joins (action "join") or leaves (action "leave") the waitlist of a full event for Noor, returning his position and the waitlist size. Use join when a class he wants is FULL and he agrees; use leave to undo. Joining opens a Chrome window for ~4 s; leaving is instant. Joining does not book a spot; the club promotes waitlisted members when a spot frees up.',
