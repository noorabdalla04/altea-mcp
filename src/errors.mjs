...
export const CODES = {
  NOT_SIGNED_IN: { retryable: false, next: 'Ask Noor to run `node ~/Projects/altea-mcp/bin/altea.mjs login` (one-time Chrome sign-in), then retry.' },
  ...
  WINDOW_NOT_OPEN: { retryable: false, next: 'Tell Noor when booking opens (48 h before start); offer to book then.' },
  LATE_CANCEL: { retryable: false, next: 'Tell Noor the fee and deadline; call again with force=true only if he confirms.' },
  ...
  UNSIGNED_AGREEMENT: { retryable: false, next: 'Noor must sign the waiver in the Altea app first; never sign on his behalf.' },
  NO_MEMBERSHIP: { retryable: false, next: 'This event is not covered by his membership; say so.' },
