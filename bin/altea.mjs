  ...
  try {
    switch (cmd) {
      ...
      case 'event': {
        ...
        for (const o of res.options) out(`Option: ${o.title} · ${o.unlimited ? 'unlimited' : `$${(o.price / 100).toFixed(2)}`} · booking ${o.bookableNow ? 'open now' : 'opens ' + o.bookableFrom} (${o.bookingWindowMin / 60} h window) · cancel ${o.cancellation?.text ?? '?'}${o.cancellation?.feeText ? ' or ' + o.cancellation.feeText : ''}${o.disabled ? ' (disabled)' : ''}`);
