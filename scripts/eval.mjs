...
await evalCase('Q1 "sessions run by Omar on Monday"', async () => { const r = await tool('altea_instructor', { name: 'omar', date: 'mon' }); if (r.isError) throw new Error(r.text); if (r.s.count < 1) throw new Error('no sessions'); if (!r.s.sessions.every((e) => e.instructors.some((n) => /omar/i.test(n)) && e.date === r.s.from)) throw new Error('wrong instructor/date'); return `${r.s.count} sessions across ${new Set(r.s.sessions.map((e) => e.group)).size} groups`; });
...
await evalCase('Q3 "pickleball courts open tomorrow at 3 pm"', async () => { const r = await tool('altea_schedule', { date: 'tomorrow', group: 'pickleball', at: '3pm', availableOnly: true }); if (r.isError) throw new Error(r.text); const evs = r.s.days[0].events; if (!evs.length) return 'none open (valid answer)'; if (!evs.every((e) => e.time <= '15:00' && e.end > '15:00' && !e.full)) throw new Error('filter leak'); return `${evs.length} courts open`; });
