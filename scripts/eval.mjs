...
await evalCase('Q1 "sessions run by Omar on Monday"', async () => { const c = await tool('altea_instructor', { name: 'omar', date: 'mon' }); if (c.isError) throw new Error(c.text); if (c.s.count < 1) throw new Error('no sessions'); if (!c.s.sessions.every((e) => /omar/i.test(e.who || '') && e.date === c.s.from)) throw new Error('concise: wrong who/date'); const r = await tool('altea_instructor', { name: 'omar', date: 'mon', format: 'detailed' }); if (!r.s.sessions.every((e) => e.instructors.some((n) => /omar/i.test(n)) && e.date === r.s.from)) throw new Error('detailed: wrong instructor/date'); return `${r.s.count} sessions across ${new Set(r.s.sessions.map((e) => e.group)).size} groups`; });
await evalCase('Q2 "spots left in the next Hot Yin"', async () => { const r = await tool('altea_next', { query: 'hot yin' }); if (r.isError || !r.s.next) throw new Error(r.text); if (!/hot yin/i.test(r.s.next.title)) throw new Error('wrong title'); if (new Date(r.s.next.date + 'T' + r.s.next.time + ':00-04:00').getTime() < nowMs - 3600_000) throw new Error('not in the future'); if (typeof r.s.waitlistedUsers !== 'number' || typeof r.s.bookableNow !== 'boolean') throw new Error('missing waitlist/window'); return `${r.s.next.date} ${r.s.next.time} ${r.s.next.full ? 'FULL' : r.s.next.spotsLeft + ' left'}${r.s.nextWithSpots && !r.s.sameEvent ? `; next with spots ${r.s.nextWithSpots.date} ${r.s.nextWithSpots.time}` : ''}`; });
await evalCase('Q3 "pickleball courts open tomorrow at 3 pm"', async () => { const r = await tool('altea_schedule', { date: 'tomorrow', group: 'courts', at: '3pm', availableOnly: true, format: 'detailed' }); if (r.isError) throw new Error(r.text); const evs = r.s.days[0].events; if (!evs.length) return 'none open (valid answer)'; if (!evs.every((e) => e.time <= '15:00' && e.end > '15:00' && !e.full)) throw new Error('filter leak'); const c = await tool('altea_schedule', { date: 'tomorrow', group: 'courts', at: '3pm', availableOnly: true }); if (!c.s.days[0].events.every((e) => typeof e.spots === 'number' && e.spots > 0)) throw new Error('concise: spots missing'); return `${evs.length} courts open`; });
await evalCase('typo → suggestion', async () => { const r = await tool('altea_instructor', { name: 'omer', date: 'mon' }); if (r.isError) throw new Error(r.text); if (r.s.count !== 0 || !r.s.suggestions.some((s) => /omar/i.test(s))) throw new Error('no suggestion'); return r.s.suggestions.join(', '); });
